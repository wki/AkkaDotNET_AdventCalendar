...
namespace SudokuSolver
{
    ...
    class MainClass
    {
        ...
        private static void CreateSudokuActors(ActorSystem system)
        {
            ...
            {
                system.ActorOf(Props.Create<SudokuRow>(printer, row), String.Format("r-{0}", row));
                for (int col = 0; col < 9; col++)
                {
                    if (row == 0)
                        system.ActorOf(Props.Create<SudokuCol>(printer, col), String.Format("c-{0}", col));
                    system.ActorOf(Props.Create<SudokuCell>(printer, row, col), String.Format("{0}-{1}", row, col));
                }
            }
                
            for (int block = 0; block < 9; block++)
                system.ActorOf(Props.Create<SudokuBlock>(printer, block), String.Format("b-{0}", block));
        }
